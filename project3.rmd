...
## Lasso  
Lasso regression can be an extension of linear regression or logistic regression. Here, we will use it with a logistic regression model since we are working with a categorical response variable.Lambda is the growth variable adn you want to increase that slowly. 
```{r}
...
print(fit.lasso)
```
Now we will select our best Lasso model using the `Accuracy` metric.
```{r}
best.lasso <- fit.lasso$results %>% 
  mutate(lambda = lambda, max = max(Accuracy)) %>% 
  filter(Accuracy == max)
...
print(fit.classtree)
```
Now we will select our best Classification Tree model using the `Accuracy` metric.
```{r}
best.classtree <- fit.classtree$results %>% 
  mutate(cp = cp, max = max(Accuracy)) %>% 
  filter(Accuracy == max)
...
Partial Least Squares is used when predictors are highly correlated and determines the best model to use that covers the most variance with the least amount of predictors.
```{r}
#create PLS model
...
print(fit.pls)
```
Now we will select our best PLS model using the `Accuracy` metric.
```{r}
best.pls <- fit.pls$results %>% 
  mutate(ncomp = ncomp, max = max(Accuracy)) %>% 
  filter(Accuracy == max)
