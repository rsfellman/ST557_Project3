...
rbind(tab.Alc, apply(tab.Alc, MARGIN = 2, FUN = function(x){round(x[2] / sum(x), 3)}))
```  

```{r}
# group by PhysHlth to create graph of proportion of people with diabetes at each number of days with illness/injury.
noDoc.sum <- diabetes.ed %>% 
  group_by(NoDocbcCost) %>% 
  summarise(proportion.diabetes = mean(Diabetes_binary), n = n())
#create plot
ggplot(noDoc.sum, aes(x = noDocbcCost, y = proportion.diabetes, size = n)) +
  geom_point(stat = "identity")+ 
#add title and labels
  xlab("Avoided Doctor")+
  labs(title = "Proportion of People with Diabetes", subtitle = "vs whether they avoided Doctor visits because of the cost")
```
