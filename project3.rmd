...
Now that we have an understanding of logistic regression we will fit tree candidate logistic regression models and then choose the best model using the log loss.

We will use the `train` function from the `caret` package to fit out models. We will set the method to `glm` and the family to `binomial` to specify binomial logistic regression. We will also preprocess pur data by centering and scaling it. For each of the three models we will do cross validation with 5 folds. 
...
print(fit.rf)
```
The best model for the random forest was with `mtry = 5`. `mtry` is a tuning parameter that gives us the number of randomly selected predictors we will use. Since the random forest model uses only a subset of the predictors and not all of them, we tested the model on all numbers of predictors in the dataset (1 to 21). 

Next we will look at some models that were not explained in class.

## Oblique Decision trees

Since we are already familiar with random forests and decision trees we will take that one step further and model the data with an oblique decision tree. Oblique decision trees are a tree model that uses a linear combination of variables at each node instead of just one variable. This allows there to be an infinite number of possible splits for the model to chose from. In the case of this project we are wanting to predict the variable `Diabetes_binary` using variables such as `HighBP`, `PhysActivity`, and `Veggies`, etc. A standard decision tree (like those used above) would only check a single variable. For example, the node might split along `HighBP` with values of 1 forming one branch and 0 the other. With the oblique model, the algorithm might instead split a node along 2(`HighBP`) + 3(`PhysActivity`) + 2(`Veggies`). Since oblique decision trees split along linear combinations of variables they do lose the interpretability of a standard tree model. 

We will fit the oblique decision tree using the `train` function from the `caret` package. We will continue to use cross validation with 5 folds. We will specify the `method = ORFpls` for oblique random forest. There appear to be a few different oblique methods to chose from and the pls in this case stands for partial least squares. We could not find information on which would be best to use in our case.
```{r, message=FALSE, warning=FALSE}
#create oblique model
fit.ob<- train(Diabetes_binary ~ . , data = diabetes.train.log,
                 #select rf method
                 method = "wsrf",
                 # calculate log loss metric
                 metric = "logLoss",
...
```{r}
a<- confusionMatrix(data = diabetes.test.log$Diabetes_binary, reference = predict(fit.rf, newdata = diabetes.test.log))
b<- confusionMatrix(data = diabetes.test.log$Diabetes_binary, reference = predict(fit.log3, newdata = diabetes.test.log))
