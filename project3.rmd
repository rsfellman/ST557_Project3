...
fit.log1 <- train(Diabetes_binary ~ AnyHealthcare + Sex + Age + HighChol, data = diabetes.train.log,
                 ...
                 metric = "logLoss",
                 #do cross validation
                 trControl = trainControl(method = "cv", number = 5, summaryFunction = mnLogLoss, classProbs = TRUE, savePredictions = TRUE)
                 )
```
...
#create the predictions for each model using the test set
pred.a<- data.frame(obs=diabetes.test.log$Diabetes_binary,pred=predict(fit.log3, newdata = diabetes.test.log),predict(fit.log3, newdata = diabetes.test.log, type="prob"))
pred.b<- data.frame(obs=diabetes.test.log$Diabetes_binary,pred=predict(fit.rf, newdata = diabetes.test.log),predict(fit.rf, newdata = diabetes.test.log, type="prob"))
pred.c<- data.frame(obs=diabetes.test.log$Diabetes_binary,pred=predict(fit.ob, newdata = diabetes.test.log),predict(fit.ob, newdata = diabetes.test.log, type="prob"))
pred.d<- data.frame(obs=diabetes.test.log$Diabetes_binary,pred=predict(fit.lasso, newdata = diabetes.test.log),predict(fit.lasso, newdata = diabetes.test.log, type="prob"))
pred.e<- data.frame(obs=diabetes.test.log$Diabetes_binary,pred=predict(fit.classtree, newdata = diabetes.test.log),predict(fit.classtree, newdata = diabetes.test.log, type="prob"))
pred.f<- data.frame(obs=diabetes.test.log$Diabetes_binary,pred=predict(fit.pls, newdata = diabetes.test.log),predict(fit.pls, newdata = diabetes.test.log, type="prob"))
```

```{r}
#calculate LogLoss using the mnLogLoss function
ll.a<- mnLogLoss(pred.a, lev = levels(pred.a$obs))
ll.b<- mnLogLoss(pred.b, lev = levels(pred.b$obs))
ll.c<- mnLogLoss(pred.c, lev = levels(pred.c$obs))
ll.d<- mnLogLoss(pred.d, lev = levels(pred.d$obs))
ll.e<- mnLogLoss(pred.e, lev = levels(pred.e$obs))
ll.f<- mnLogLoss(pred.f, lev = levels(pred.f$obs))
```  
...
log_loss<- c(ll.a,ll.b,ll.c,ll.d,ll.e,ll.f)
all.fits<- tibble(name, accuracy, log_loss)
all.fits
```
...
```{r}
best<- all.fits %>% 
  mutate(name== name, best = min(log_loss)) %>% 
  filter(best == log_loss)
```
