...
pred.a <- predict(fit.log3, newdata = diabetes.test.log)
...
pred.a<- predict(fit.log3, newdata = diabetes.test.log)
pred.b<- predict(fit.rf, newdata = diabetes.test.log)
pred.c<- predict(fit.ob, newdata = diabetes.test.log)
pred.d <- predict(fit.lasso, newdata = diabetes.test.log)
pred.e<- predict(fit.classtree, newdata = diabetes.test.log)
pred.f<- predict(fit.pls, newdata = diabetes.test.log)
```

```{r}
#create function for calculating LogLoss
...
diabetes.test.log$diabetesLL <- as.numeric(ifelse(diabetes.test.log$Diabetes_binary == "no", 0, 1))
pred.a<- as.numeric(ifelse (pred.a=="no",0,1))
pred.b<- as.numeric(ifelse (pred.b=="no",0,1))
pred.c<- as.numeric(ifelse (pred.c=="no",0,1))
pred.d<- as.numeric(ifelse (pred.d=="no",0,1))
pred.e<- as.numeric(ifelse (pred.e=="no",0,1))
pred.f<- as.numeric(ifelse (pred.f=="no",0,1))
#calculate LogLoss using the function
ll.a<- LogLoss(diabetes.test.log$diabetesLL,pred.a)
ll.b<- LogLoss(diabetes.test.log$diabetesLL,pred.b)
ll.c<- LogLoss(diabetes.test.log$diabetesLL,pred.c)
ll.d<- LogLoss(diabetes.test.log$diabetesLL,pred.d)
ll.e<- LogLoss(diabetes.test.log$diabetesLL,pred.e)
ll.f<- LogLoss(diabetes.test.log$diabetesLL,pred.f)
```  
