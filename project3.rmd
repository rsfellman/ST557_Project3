...
```
`mtry` is a tuning parameter that gives us the number of randomly selected predictors we will use. Since the random forest model uses only a subset of the predictors and not all of them, we tested the model on all numbers of predictors in the dataset (1 to 21). 
...
We will fit the oblique decision tree using the `train` function from the `caret` package. We will continue to use cross validation with 5 folds. We will specify the `method = LMT`. We are choosing to do 1 to 5 iterations as our tuning parameter.
